...
int get_baudRateValMacro(int baudRate) {
	int baudRateMacro;
	switch(baudRate) {
...
void configureTermios(serial_obj_t *serial) {
	struct termios tty;
	...
	//set timeout
    if(serial->timeOutms == -1) {
        // Always wait for at least one byte, this could
        // block indefinitely
        tty.c_cc[VTIME] = 0;
        tty.c_cc[VMIN] = 1;
    } else if(serial->timeOutms == 0) {
        // Setting both to 0 will give a non-blocking read
        tty.c_cc[VTIME] = 0;
        tty.c_cc[VMIN] = 0;
    } else if(serial->timeOutms > 0) {
        tty.c_cc[VTIME] = (cc_t)(serial->timeOutms/100);    // 0.5 seconds read timeout
        tty.c_cc[VMIN] = 0;
    }

    // Set in/out baud rate
    int baudRate = get_baudRateValMacro(serial->baudRate);
...
void Serial_setConfig(serial_obj_t *serial, char const *ser, int baudRate) {
	...
	strcpy(serial->serDevice, ser);
	serial->fd = -1;
	serial->flowControl = 0; //0 None / 1 Enable RTS/CTS hardware flow control
	serial->stopBits = 1;
	serial->dataBits = 8;
}

void Serial_open(serial_obj_t *serial, int timeOut) {
	serial->fd = open(serial->serDevice, O_RDWR | O_NOCTTY | O_SYNC);
	if (serial->fd < 0) {
	    printf("Error %i from open: %s\n", errno, strerror(errno));
	    return;
	}

	if (serial->fd == 2) { //No such file or directory
		printf("Error %i from open: %s\n", errno, strerror(errno));
		return;
	}

	if (serial->fd == 13) { //Permission denied
		printf("Error %i from open: %s\n", errno, strerror(errno));
		return;
	}

	serial->timeOutms = timeOut;

	configureTermios(serial);
...
int Serial_read(serial_obj_t *serial,char *buff, int len){
	...
	ssize_t n = read(serial->fd, buff, len);
...
int Serial_write(serial_obj_t *serial,char *buff, int len) {
	...
	int writtenBytes = write(serial->fd, buff, len);
